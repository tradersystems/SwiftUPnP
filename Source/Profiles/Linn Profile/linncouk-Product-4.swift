...
public class linncoukProduct4Service: UPnPService {
	...
	public enum A_ARG_TYPE_GetImageUri_ResolutionEnum: String, Codable {
		case high = "High"
		case low = "Low"
	}
	...
	public struct ProductResponse: Codable {
		enum CodingKeys: String, CodingKey {
			...
			case url = "Url"
			case imageUri = "ImageUri"
			case imageHiresUri = "ImageHiresUri"
		}
		...
		public var url: String
		public var imageUri: String
		public var imageHiresUri: String
		...
		public func log(deep: Bool = false, indent: Int = 0) {
			...
			Logger.swiftUPnP.debug("\(Logger.indent(indent+1))url: '\(url)'")
			Logger.swiftUPnP.debug("\(Logger.indent(indent+1))imageUri: '\(imageUri)'")
			Logger.swiftUPnP.debug("\(Logger.indent(indent+1))imageHiresUri: '\(imageHiresUri)'")
			Logger.swiftUPnP.debug("\(Logger.indent(indent))}")
		}
	}
	...
	public struct StandbyTransitioningResponse: Codable {
		enum CodingKeys: String, CodingKey {
			case value = "Value"
		}
		...
		public func log(deep: Bool = false, indent: Int = 0) {
			Logger.swiftUPnP.debug("\(Logger.indent(indent))StandbyTransitioningResponse {")
			Logger.swiftUPnP.debug("\(Logger.indent(indent+1))value: \(value == true ? "true" : "false")")
			Logger.swiftUPnP.debug("\(Logger.indent(indent))}")
		}
	}
	public func standbyTransitioning(log: UPnPService.MessageLog = .none) async throws -> StandbyTransitioningResponse {
		struct SoapAction: Codable {
			enum CodingKeys: String, CodingKey {
				case urn = "xmlns:u"
			}
		...
		struct Body: Codable {
			enum CodingKeys: String, CodingKey {
				case action = "u:StandbyTransitioning"
				case response = "u:StandbyTransitioningResponse"
			}

			var action: SoapAction?
			var response: StandbyTransitioningResponse?
		}
		let result: Envelope<Body> = try await postWithResult(action: "StandbyTransitioning", envelope: Envelope(body: Body(action: SoapAction(urn: Attribute(serviceType)))), log: log)
	...
	public struct GetImageUriResponse: Codable {
		enum CodingKeys: String, CodingKey {
			case imageUri = "ImageUri"
		}
		...
		public func log(deep: Bool = false, indent: Int = 0) {
			Logger.swiftUPnP.debug("\(Logger.indent(indent))GetImageUriResponse {")
			Logger.swiftUPnP.debug("\(Logger.indent(indent+1))imageUri: '\(imageUri)'")
			Logger.swiftUPnP.debug("\(Logger.indent(indent))}")
		}
	}
	public func getImageUri(resolution: A_ARG_TYPE_GetImageUri_ResolutionEnum, log: UPnPService.MessageLog = .none) async throws -> GetImageUriResponse {
		struct SoapAction: Codable {
			enum CodingKeys: String, CodingKey {
				case urn = "xmlns:u"
			...
			@Attribute var urn: String
			public var resolution: A_ARG_TYPE_GetImageUri_ResolutionEnum
		}
		struct Body: Codable {
			enum CodingKeys: String, CodingKey {
				case action = "u:GetImageUri"
				case response = "u:GetImageUriResponse"
			}

			var action: SoapAction?
			var response: GetImageUriResponse?
		}
		let result: Envelope<Body> = try await postWithResult(action: "GetImageUri", envelope: Envelope(body: Body(action: SoapAction(urn: Attribute(serviceType), resolution: resolution))), log: log)
...
// Event parser
extension linncoukProduct4Service {
	public struct State: Codable {
		enum CodingKeys: String, CodingKey {
			...
			case productImageHiresUri = "ProductImageHiresUri"
			case standby = "Standby"
			case standbyTransitioning = "StandbyTransitioning"
		...
		public var productImageHiresUri: String?
		public var standby: Bool?
		public var standbyTransitioning: Bool?
		...
		public func log(deep: Bool = false, indent: Int = 0) {
			Logger.swiftUPnP.debug("\(Logger.indent(indent))linncouk-Product-4State {")
			...
			Logger.swiftUPnP.debug("\(Logger.indent(indent+1))productImageHiresUri: '\(productImageHiresUri ?? "nil")'")
			Logger.swiftUPnP.debug("\(Logger.indent(indent+1))standby: \((standby == nil) ? "nil" : (standby! == true ? "true" : "false"))")
			Logger.swiftUPnP.debug("\(Logger.indent(indent+1))standbyTransitioning: \((standbyTransitioning == nil) ? "nil" : (standbyTransitioning! == true ? "true" : "false"))")
	...
	public func state(xml: Data) throws -> State {
		...
		return propertySet.property.reduce(State()) { partialResult, property in
			...
			if let productImageHiresUri = property.productImageHiresUri {
				result.productImageHiresUri = productImageHiresUri
			}
			if let standby = property.standby {
				result.standby = standby
			}
			if let standbyTransitioning = property.standbyTransitioning {
				result.standbyTransitioning = standbyTransitioning
			}
